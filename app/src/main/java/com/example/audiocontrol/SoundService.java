...
import com.example.audiocontrol.util.NotificationWidgetUpdateTracker;
import com.example.audiocontrol.util.ProfileApplier;
...
public class SoundService extends Service {
    ...
    private AudioControl.AudioControlListener audioControlListener = ($$$, $$, $) -> {
        updateNotification();
    };


    private static RemoteViews buildControlValueSlider(Context context, AudioControl control, int typeId, String typeName) {
        ...
        int currentLevel = control.getCurrentLevel(typeId);
    ...
    private static Notification buildForegroundNotification(
        ...
        if (volumeTypesToShow != null) {
            remoteViews.removeAllViews(R.id.audio_control_sliders);

            for (AudioType notificationType : AudioType.getAudioTypes()) {
                if (volumeTypesToShow.contains(notificationType.nameId)) {
                    remoteViews.addView(R.id.audio_control_sliders, buildControlValueSlider(context, control, notificationType.nameId, context.getString(notificationType.nameId)));
                }
            }
    ...
    private void registerListeners(List<Integer> profilesToShow) {
        ...
        if (profilesToShow != null) {
            for (Integer id : profilesToShow) {
                control.registerAudioControlListener(id, audioControlListener, false);
            }
        }
    ...
    public void onCreate() {
        ...
        soundProfileStorage = com.example.audiocontrol.SoundApplication.getSoundProfileStorage(this);
